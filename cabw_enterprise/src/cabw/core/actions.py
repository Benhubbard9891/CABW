...
class ActionContext:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.agent is not None and hasattr(self.agent, 'stats'):
            resources.setdefault('stamina', getattr(self.agent.stats, 'energy', 0.0))
        resources.setdefault('action_points', 1.0)
        resources.setdefault('inventory', self.inventory)
...
class ComplexAction:
    ...
    def execute(self, context: dict[str, Any] | ActionContext) -> dict[str, Any]:
        ...
        # Check can execute
        can_exec, failures = self.can_execute(context_dict)
        if not can_exec:
            return {
                'success': False,
...
class ActionLibrary:
    ...
    def __init__(self):
        self._actions: dict[str, ComplexAction] = {
            ...
            'coordinate': self.create_coordinate_action(),
