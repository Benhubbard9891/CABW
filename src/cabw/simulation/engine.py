...
from ..core.teamwork import GoalTemplates, SharedGoal, TeamManager
from ..core.world_features import EnvironmentEffectSystem, WorldEnvironment
from ..governance.security import Capability, SecurityGovernor, SecurityPolicy
...
class EnhancedSimulation:
    ...
    def _setup_security(self) -> SecurityGovernor:
        ...
        default_policy = SecurityPolicy(
            ...
            rules=[lambda s, r, c, ctx: (True, None)],  # Allow all by default
    ...
    def add_agent(
        ...
        agent = IntegratedAgent(name=name, ocean_traits=ocean_traits, initial_location=location)
        ...
        # Log
        self._log_event(
            "agent_added", {"agent_id": agent.agent_id, "name": name, "location": location}
        )
    ...
    def initialize(self):
        ...
        for i in range(self.config.num_agents):
            ...
            agent = self.add_agent(name=f"Agent_{i+1}", ocean_traits=base_traits)
        ...
        self._log_event(
            ...
            {"num_agents": len(self.agents), "world_size": self.config.world_size},
        )
    ...
    async def run(self):
        ...
        self.running = False
        self._log_event(
            "simulation_ended", {"total_ticks": self.tick_count, "statistics": self.statistics}
        )
    ...
    async def tick(self) -> dict[str, Any]:
        ...
        agent_emotions = {
            agent_id: agent.emotional_state for agent_id, agent in self.agents.items()
        }
    ...
    def _get_nearby_agents(self, agent_id: str, radius: float = 3.0) -> list[IntegratedAgent]:
            ...
            if hazard.active and hazard.get_requires_teamwork() and not hazard.contained:
                ...
                for _agent_id, agent in self.agents.items():
                    if agent.stats.is_alive() and not agent.current_team:
                        distance = (
                            (agent.location[0] - hazard.location[0]) ** 2
                            + (agent.location[1] - hazard.location[1]) ** 2
                        ) ** 0.5
                        if distance <= 5:
        ...
        for event in self.environment.events.values():
            if event.active and event.requires_teamwork and not event.resolved:
                ...
                if not has_team:
                    ...
                    goal = GoalTemplates.emergency_response(f"Respond to {event.name}", urgency=0.8)
                    self.team_manager.assign_goal_to_team(goal)

    def create_team_goal(self, team_id: str, goal_type: str, **params) -> SharedGoal | None:
        ...
        if self.team_manager.assign_goal_to_team(goal, team_id):
            self._log_event(
                "goal_assigned",
                {"team_id": team_id, "goal_type": goal_type, "goal_id": goal.goal_id},
            )
            return goal
    ...
    def trigger_event(self, event_type: str, **kwargs) -> Any:
        ...
        self._log_event(
            "manual_event_triggered", {"event_type": event_type, "event_id": event.event_id}
        )
        return event
    ...
    def get_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "agents": {
                agent_id: agent.get_state_summary() for agent_id, agent in self.agents.items()
            },
            "teams": {
                team_id: {
