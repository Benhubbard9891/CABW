...
class APIAuthManager:
    ...
    def _setup_policies(self):
        ...
        admin_policy = SecurityPolicy(
            ...
            rules=[lambda s, r, c, ctx: (s.role == APIRole.ADMIN, "Admin role required")],
    ...
    def create_user(
        self, username: str, password: str, role: APIRole, simulation_access: list[str] = None
    ) -> str:
    ...
    def create_access_token(
        self, principal: APIPrincipal, expires_delta: timedelta | None = None
    ) -> str:
    ...
    def check_api_access(
        self, principal: APIPrincipal, action: str, simulation_id: str | None = None
    ) -> tuple[bool, str | None]:
        """
...
async def require_simulation_access(sim_id: str):
    ...
    async def checker(principal: APIPrincipal = Depends(get_current_principal)):
        if not principal.can_access_simulation(sim_id):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Access denied to simulation"
            )
        return principal
...
def create_default_users():
    ...
    auth_manager.create_user(username="viewer", password="viewer123", role=APIRole.VIEWER)

    auth_manager.create_user(username="operator", password="operator123", role=APIRole.OPERATOR)

    auth_manager.create_user(username="admin", password="admin123", role=APIRole.ADMIN)
