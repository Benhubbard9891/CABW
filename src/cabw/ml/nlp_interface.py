...
class CommandProcessor:
    ...
    PATTERNS = {
        ...
        "get_status": [r"what is (?:the )?status", r"show status", r"get simulation state"],
        "export_results": [r"export results", r"save simulation", r"download data"],
    }
    ...
    def execute_command(self, command: ParsedCommand, simulation: Any) -> dict[str, Any]:
        """
        Execute parsed command on simulation.
        """
        result = {"success": False, "action": command.action, "message": ""}
...
class AgentDialogue:
    ...
    def generate_utterance(self, dialogue_type: str, target_agent: Any | None = None) -> str:
        """
    ...
    def respond_to(self, message: str, sender: Any, context: dict | None = None) -> str:
        """
...
class NLPInterface:
    ...
    def process_user_input(self, text: str, user_id: str = "user") -> dict[str, Any]:
        """
        ...
        if command and command.confidence > 0.5:
            # Execute command
            result = self.command_processor.execute_command(command, self.simulation)

            self.conversation_history.append(
                {"speaker": user_id, "text": text, "type": "command", "result": result}
            )

            return {"type": "command_result", "command": command.action, "result": result}

        else:
            # Treat as dialogue
            response = self._generate_system_response(text)

            self.conversation_history.append({"speaker": user_id, "text": text, "type": "dialogue"})

            return {"type": "dialogue", "response": response}

    def agent_speak(
        self, agent_id: str, dialogue_type: str, target_agent_id: str | None = None
    ) -> str:
        """
        ...
        utterance = dialogue.generate_utterance(dialogue_type, target.agent if target else None)
    ...
    def agent_respond(self, agent_id: str, message: str, sender_id: str) -> str:
        """
        ...
        response = dialogue.respond_to(message, sender.agent if sender else None)

        self.conversation_history.append(
            {"speaker": agent_id, "text": response, "type": "agent_response", "target": sender_id}
        )
