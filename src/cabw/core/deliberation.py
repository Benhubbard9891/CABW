...
class DeliberationEngine:
    ...
    def register_calculator(
        self, factor: DeliberationFactor, calculator: Callable[[Any, ComplexAction, Any], float]
    ):
        """Register a factor calculator."""
        self._calculators[factor] = calculator

    def score_action(self, agent, action: ComplexAction, context: Any = None) -> ActionScore:
        """
        ...
        return ActionScore(action=action, total_score=total, factors=factors, reasoning=reasoning)
    ...
    def _score_environment(self, agent, action: ComplexAction, context: Any) -> float:
        ...
        if hasattr(context, "weather"):
            ...
            if weather.weather_type.name in ["STORM", "BLIZZARD"] and getattr(
                action, "outdoor", False
            ):
...
class DeliberationLogger:
    ...
    def log_decision(
        self, agent_id: str, selected: ActionScore, candidates: list[ActionScore], tick: int
    ):
        ...
        self.decisions.append(
            {
                ...
                "selected_factors": {k.name: v for k, v in selected.factors.items()},
                "candidate_count": len(candidates),
                "candidate_scores": [
                    {"action": c.action.name, "score": c.total_score} for c in candidates
                ],
