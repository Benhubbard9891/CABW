...
class DeliberationEngine:
    ...
    def _score_emotion(self, agent, action: ComplexAction) -> float:
        ...
        if emotions.get_valence() < -0.3:
            if action_emotions.get('safety', 0) > 0.5:
                score += 0.2
        
        # Fear → prefer escape/defense
        if emotions.fear > 0.5:
            if action_emotions.get('defensive', 0) > 0.5:
                score += 0.3
        
        # Anger → prefer confrontational actions
        if emotions.anger > 0.5:
            if action_emotions.get('confrontational', 0) > 0.5:
                score += 0.2
    ...
    def _score_environment(
        ...
        if hasattr(context, 'get_visibility'):
            ...
            if visibility < 0.3:
                ...
                if getattr(action, 'risky', False):
                    score -= 0.2
        
        # Weather effects
        if hasattr(context, 'weather'):
            weather = context.weather
            if weather.weather_type.name in ['STORM', 'BLIZZARD']:
                # Bad weather → prefer indoor/shelter actions
                if getattr(action, 'outdoor', False):
                    score -= 0.15
