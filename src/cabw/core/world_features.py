...
class WorldEnvironment:
        ...
        for hazard in self.hazards.values():
            if hazard.active:
                for emotion, value in hazard.emotional_impact.items():
                    modifiers[emotion] = (
                        modifiers.get(emotion, 0.0) + value * hazard.severity.value / 5
                    )
    ...
    def _transition_weather(self):
        ...
        self.weather = WeatherState(
            ...
            wind_speed=(
                random.uniform(0, 50)
                if new_type in [WeatherType.STORM, WeatherType.BLIZZARD]
                else random.uniform(0, 20)
            ),
        )
    ...
    def get_active_threats(self, location: tuple[int, int] | None = None) -> list[Hazard]:
        ...
        if location:
            threats = [
                ...
                for h in threats
                if math.sqrt(
                    (h.location[0] - location[0]) ** 2 + (h.location[1] - location[1]) ** 2
                )
                <= h.radius
            ]
